...
This project is a Cisco Packet Tracer simulation represents a secure and scalable two-campus network infrastructure for Abdullah Gül University.

The network separates departments into different VLANs, provides controlled inter-VLAN communication, and implements routing, DHCP, ACLs, SSH, wireless access, and port security.
...
A centralized DHCP server is located in VLAN 60. Separate DHCP pools are configured for each VLAN.

Branch Campus clients receive IP addresses from the Main Campus DHCP server through DHCP relay using `ip helper-address`.
